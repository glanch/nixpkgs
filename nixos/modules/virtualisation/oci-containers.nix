...
let
  ...
  mkService = name: container:
    ...
    in
    {
      ...
      after = lib.optionals (cfg.backend == "docker") [ "docker.service" "docker.socket" ] ++ dependsOn ++ container.systemdUnit.after;
      requires = dependsOn ++ container.systemdUnit.requires;
